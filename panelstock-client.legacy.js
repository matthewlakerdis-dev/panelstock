...
(function (root) {
  ...
  function styledConfirm({
    ...
    return new Promise(resolve => {
      ...
      confirm.style.cssText = `min-height:42px;border:0;border-radius:8px;background:${danger ? '#b91c1c' : '#0e7490'};padding:10px 18px;color:#fff;font:600 14px inherit;cursor:pointer`;
  ...
  function showCopyDialog({
    title = 'Copy',
    message = 'Select and copy this value.',
    value = ''
  } = {}) {
    return new Promise(resolve => {
      const document = root.document;
  ...
  class IndexedOutboxStorage {
    ...
    async ready() {
      ...
      try {
        this.db = await new Promise((resolve, reject) => {
          const request = root.indexedDB.open('panelstock-sync', 1);
          request.onupgradeneeded = () => request.result.createObjectStore('state');
          request.onsuccess = () => resolve(request.result);
          request.onerror = () => reject(request.error);
        });
        const saved = await new Promise((resolve, reject) => {
          const request = this.db.transaction('state').objectStore('state').get(OUTBOX_KEY);
          request.onsuccess = () => resolve(request.result ?? null);
          request.onerror = () => reject(request.error);
        });
        const legacy = this.fallback.getItem(OUTBOX_KEY);
        this.value = saved ?? legacy;
    ...
    setItem(key, value) {
      ...
      this.writes = this.writes.then(() => new Promise((resolve, reject) => {
        ...
        try {
          this.fallback.setItem(key, this.value);
        } catch {}
        this.notify('storage', 'Reliable device storage failed. Pending changes were preserved in limited browser storage.');
      });
    }
    removeItem(key) {
      if (key !== OUTBOX_KEY) {
    ...
    async read(key) {
      await this.flushWrites();
      if (!this.db) return key === SESSION ? null : this.fallback.getItem(key);
  ...
  class Outbox {
    ...
    finalize() {
      ...
      for (const [field, value] of Object.entries(next.draft.fields)) {
        const before = mapping(field, next.draft.before[field]),
    ...
    flush(owner) {
      ...
      this.running = this.drain(owner).finally(() => {
        this.running = null;
      });
      return this.running;
    }
  ...
  Array.prototype.sort = function (compareFn) {
    ...
    if (isCncPanelArray && isUploadedAtSort) {
      return nativeArraySort.call(this, (a, b) => {
        const sheet = String(a.sheetNumber ?? '').trim().localeCompare(String(b.sheetNumber ?? '').trim(), 'en', {
          numeric: true,
          sensitivity: 'base'
        });
        if (sheet) return sheet;
        return String(a.panelNumber ?? '').trim().localeCompare(String(b.panelNumber ?? '').trim(), 'en', {
          numeric: true,
          sensitivity: 'base'
        });
      });
  ...
  const addMobileSheetGroups = () => {
    ...
    for (const parent of parents) {
      ...
      for (const sheet of sheets) {
        ...
        const update = () => {
          const closed = collapsedCncSheets.has(key);
          right.textContent = sheetCards.length + ' panel' + (sheetCards.length === 1 ? '' : 's') + (closed ? ' ▸' : ' ▾');
          for (const card of sheetCards) card.style.display = closed ? 'none' : '';
        };
        heading.onclick = () => {
    ...
    for (const table of document.querySelectorAll('table')) {
      ...
      for (const row of rows) {
        const cells = [...row.children],
          sheet = cells[sheetIndex]?.textContent.trim();
  ...
  const enhanceCncHierarchy = () => {
    addMobileSheetGroups();
    addDesktopSheetGroups();
  };
  const queueCncEnhance = () => {
  ...
  const startLive = async () => {
    clearTimeout(liveTimer);
    if (!root.WebSocket || !session || !workerUrl || liveSocket?.readyState === WebSocket.OPEN || liveSocket?.readyState === WebSocket.CONNECTING) return;
    try {
      const response = await apiFetch(workerUrl + '/live-ticket'),
        result = await response.json();
      if (!response.ok || !result.ticket) throw Error();
      const url = new URL(workerUrl);
      url.protocol = url.protocol === 'https:' ? 'wss:' : 'ws:';
      url.pathname = '/live';
      url.search = '?ticket=' + encodeURIComponent(result.ticket);
      const socket = liveSocket = new WebSocket(url);
      socket.onopen = () => {
        liveRetry = 1000;
      };
      socket.onmessage = event => {
        try {
          ...
          if (['ready', 'revision'].includes(update.type) && update.revision > (outbox?.state.view?.revision || 0)) root.dispatchEvent(new CustomEvent('panelstock-remote-change', {
            detail: update
          }));
        } catch {}
      };
      socket.onclose = () => {
        ...
        if (session) liveTimer = setTimeout(() => void startLive(), liveRetry = Math.min(liveRetry * 2, 30000));
      };
  ...
  const outboxReady = durableStorage.ready().then(() => {
    outbox = new Outbox(durableStorage, (packet, owner) => {
  ...
  function appendReview(el, legacy) {
    ...
    if (legacy) intro.textContent = 'These changes were saved by an older PanelStock version. They are preserved below exactly as stored so they can be reconciled safely.';else intro.textContent = `${info.changes} item change${info.changes === 1 ? '' : 's'} across ${info.packets} saved batch${info.packets === 1 ? '' : 'es'}.`;
  ...
  function renderNotice() {
    ...
    statusBox.textContent = legacy ? 'Recovery status: previous-version changes detected' : `Recovery status: ${info.changes} saved change${info.changes === 1 ? '' : 's'}${info.blocked ? ' — server review required' : ' — waiting to sync'}`;
    ...
    if (!legacy && !ownerMismatch && session) {
      actions.appendChild(makeButton('Retry sync now', async event => {
        ...
        if (ok && !outbox.pending()) setTimeout(() => location.reload(), 350);else renderNotice();
      }, true));
    }
    ...
    actions.appendChild(makeButton('Discard local changes', async () => {
      const warning = legacy ? 'These previous-version changes will NOT be applied to shared stock. Export a backup first if they may still be needed. Discard them now?' : 'These unsynced changes will NOT be applied to shared stock. Export a backup first if they may still be needed. Discard them now?';
  ...
  root.PanelStock = {
    ...
    async init(url) {
      ...
      if (!session) {
        try {
          await sessionWrites;
          const saved = JSON.parse((await durableStorage.read(SESSION)) || 'null');
          if (initialVersion !== sessionVersion) return null;
          session = saved?.expiresAt > Date.now() ? saved : null;
    ...
    async flush() {
      await outboxReady;
      return session ? outbox.flush(session.username) : false;
    },
    async logout() {
    ...
    get revision() {
      return outbox?.state.view?.revision;
    },
    get status() {
      return status;
    },
    get pending() {
      return !!outbox?.pending() || !!getLegacyPending();
    }
  };

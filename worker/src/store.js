...
export class InventoryStore extends DurableObject {
  constructor(ctx,env) {
    ...
    if(!this.read('qa-settings'))this.write('qa-settings',{enabledFrom:new Date().toISOString(),requireFailurePhoto:true,preventSelfApproval:true,allowSelfApprovalOverride:true,requireResolvedOrderForDispatch:true,allowDispatchOverride:true,requireDispatchPhoto:false,requireDestination:true,requireTransport:true,requireDriver:true,activePanelChecks:QA_CHECKLISTS.panel.map(([key])=>key),activeMetalworkChecks:QA_CHECKLISTS.metalwork.map(([key])=>key)});
  ...
  qaSettings(){const saved=this.read('qa-settings',{}),keys=(kind,name)=>{const available=QA_CHECKLISTS[kind].map(([key])=>key),selected=Array.isArray(saved[name])?saved[name].filter(key=>available.includes(key)):available;return selected.length?selected:available;};return {enabledFrom:saved.enabledFrom||new Date().toISOString(),requireFailurePhoto:saved.requireFailurePhoto!==false,preventSelfApproval:saved.preventSelfApproval!==false,allowSelfApprovalOverride:saved.allowSelfApprovalOverride!==false,requireResolvedOrderForDispatch:saved.requireResolvedOrderForDispatch!==false,allowDispatchOverride:saved.allowDispatchOverride!==false,requireDispatchPhoto:saved.requireDispatchPhoto===true,requireDestination:saved.requireDestination!==false,requireTransport:saved.requireTransport!==false,requireDriver:saved.requireDriver!==false,activePanelChecks:keys('panel','activePanelChecks'),activeMetalworkChecks:keys('metalwork','activeMetalworkChecks')};}
  qaChecklists(settings=this.qaSettings()){return {panel:QA_CHECKLISTS.panel.filter(([key])=>settings.activePanelChecks.includes(key)),metalwork:QA_CHECKLISTS.metalwork.filter(([key])=>settings.activeMetalworkChecks.includes(key))};}
  qaSettingsFor(actor){check(actor.isAdmin,'Admin access required',403);return {settings:this.qaSettings(),availableChecklists:QA_CHECKLISTS};}
  updateQaSettings(body,actor){check(actor.isAdmin,'Admin access required',403);const current=this.qaSettings(),valid=(kind,value)=>{const available=new Set(QA_CHECKLISTS[kind].map(([key])=>key));check(Array.isArray(value)&&value.length>0,`Select at least one ${kind} check`);const selected=[...new Set(value.map(key=>cleanText(key,50)).filter(key=>available.has(key)))];check(selected.length===value.length,`Invalid ${kind} checklist`);return selected;},booleanFields=['requireFailurePhoto','preventSelfApproval','allowSelfApprovalOverride','requireResolvedOrderForDispatch','allowDispatchOverride','requireDispatchPhoto','requireDestination','requireTransport','requireDriver'];for(const key of booleanFields)check(typeof body[key]==='boolean',`Invalid ${key} setting`);const settings={enabledFrom:current.enabledFrom,...Object.fromEntries(booleanFields.map(key=>[key,body[key]])),activePanelChecks:valid('panel',body.activePanelChecks),activeMetalworkChecks:valid('metalwork',body.activeMetalworkChecks)};this.ctx.storage.transactionSync(()=>{this.write('qa-settings',settings);this.audit(actor.username,'qa-settings',{...settings,enabledFrom:undefined});});return ok({ok:true,settings,availableChecklists:QA_CHECKLISTS});}

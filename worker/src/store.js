...
const TASKS=[
  ...
  ['factory.qa.dispatch','Dispatch QA-approved work','factory',0],
...
const QA_CHECKLISTS={
  panel:[
    ['material','Correct material and colour'],['dimensions','Correct dimensions'],['machining','Correct machining and toolpaths'],['folds','Folds or returns are correct'],['finish','Finish is free from damage'],['film','Protective film is intact'],['label','Panel ID and label are correct'],['quantity','Quantity is correct']
  ],
  metalwork:[
    ['dimensions','Correct dimensions'],['material','Correct material'],['welds','Welds are acceptable'],['finish','Finish and colour are correct'],['fixings','Required fixings are included'],['damage','Item is free from damage'],['quantity','Quantity is correct']
  ]
};
...
export class InventoryStore extends DurableObject {
  ...
  async sendPush(item){const subscriptions=this.pushSubscriptions(item.recipient);if(!subscriptions.length)return;webpush.setVapidDetails(this.env.VAPID_SUBJECT||'mailto:support@panelstockhq.com',this.env.VAPID_PUBLIC_KEY,this.env.VAPID_PRIVATE_KEY);const expired=[];await Promise.all(subscriptions.map(async subscription=>{try{await webpush.sendNotification(subscription,JSON.stringify({title:item.title,body:item.message,link:item.link||'notifications',tag:item.id}),{TTL:86400,urgency:item.priority==='urgent'?'high':'normal'});}catch(error){if(error?.statusCode===404||error?.statusCode===410)expired.push(subscription.endpoint);}}));if(expired.length)this.write('push-subscriptions',this.read('push-subscriptions',[]).filter(value=>!expired.includes(value.endpoint)));}
  markNotifications(body,actor){const id=cleanText(body.id,100),items=this.read('notifications',[]);for(let i=0;i<items.length;i++)if(items[i].recipient===actor.username&&(!id||items[i].id===id))items[i]={...items[i],readBy:[...new Set([...(items[i].readBy||[]),actor.username])]};this.ctx.storage.transactionSync(()=>this.write('notifications',items));return ok({ok:true,notifications:this.notificationsFor(actor)});}
  clearNotifications(actor){const items=this.read('notifications',[]).filter(item=>item.recipient!==actor.username);this.ctx.storage.transactionSync(()=>this.write('notifications',items));return ok({ok:true,notifications:[]});}
  adminUsernames(){return Object.entries(this.read('users',{})).filter(([,user])=>user.isAdmin&&user.active!==false).map(([username])=>username);}
  requestPasscodeReset(body,ip){this.consumeLimit('passcode-reset:'+ip,5);const username=normalizeUsername(body.username),user=this.read('users',{})[username];if(user&&user.active!==false){this.ctx.storage.transactionSync(()=>{this.notify(this.adminUsernames().filter(admin=>admin!==username),{title:'Passcode reset requested',message:`${username} has requested help resetting their passcode.`,kind:'support',priority:'important',link:'access'});this.audit(username,'passcode-reset-requested',{});});}return ok({ok:true,message:'If the account exists, the administrators have been notified.'});}
  createLiveTicket(actor){const now=Date.now(),ticket=crypto.randomUUID(),tickets=this.read('live-tickets',{});for(const [key,value] of Object.entries(tickets))if(value.expiresAt<=now)delete tickets[key];tickets[ticket]={username:actor.username,expiresAt:now+60000};this.write('live-tickets',tickets);return ok({ok:true,ticket,expiresAt:now+60000});}
  async fetch(request){const ticket=new URL(request.url).searchParams.get('ticket')||'',tickets=this.read('live-tickets',{}),entry=tickets[ticket];if(!entry||entry.expiresAt<=Date.now())return new Response('Live sync ticket expired',{status:401});delete tickets[ticket];this.write('live-tickets',tickets);if(request.headers.get('Upgrade')?.toLowerCase()!=='websocket')return new Response('WebSocket upgrade required',{status:426});const pair=new WebSocketPair(),[client,server]=Object.values(pair);this.ctx.acceptWebSocket(server);server.serializeAttachment({username:entry.username});server.send(JSON.stringify({type:'ready',revision:this.read('revision',0)}));return new Response(null,{status:101,webSocket:client});}
  broadcastRevision(revision){const message=JSON.stringify({type:'revision',revision});for(const socket of this.ctx.getWebSockets())try{socket.send(message);}catch{try{socket.close(1011,'Refresh connection');}catch{}}}
  supportTicketsFor(actor){const tickets=this.read('support-tickets',[]);return (actor.isAdmin?tickets:tickets.filter(ticket=>ticket.createdBy===actor.username)).slice().sort((a,b)=>Date.parse(b.updatedAt)-Date.parse(a.updatedAt));}
  createSupportTicket(body,actor){const subject=cleanText(body.subject,140),category=cleanText(body.category,40),priority=cleanText(body.priority,20),description=cleanText(body.description,5000),photo=supportPhoto(body.photo);check(subject,'Subject is required');check(['General','Technical issue','Account access','Feature request'].includes(category),'Select a support category');check(['Low','Normal','High','Urgent'].includes(priority),'Select a priority');check(description,'Describe what you need help with');const now=new Date().toISOString(),ticket={id:crypto.randomUUID(),subject,category,priority,description,photo,status:'Open',createdBy:actor.username,createdAt:now,updatedAt:now,messages:[]},tickets=this.read('support-tickets',[]);tickets.unshift(ticket);this.ctx.storage.transactionSync(()=>{this.write('support-tickets',tickets);this.notify(this.adminUsernames().filter(username=>username!==actor.username),{title:'New support ticket',message:`${actor.username}: ${subject}`,kind:'support',priority:priority==='Urgent'?'urgent':priority==='High'?'important':'normal',link:'support'});this.audit(actor.username,'support-ticket-created',{ticketId:ticket.id,category,priority});});return ok({ok:true,ticket,tickets:this.supportTicketsFor(actor)},201);}
  supportTicket(id,actor){const tickets=this.read('support-tickets',[]),index=tickets.findIndex(ticket=>ticket.id===id);check(index>=0,'Support ticket not found',404);check(actor.isAdmin||tickets[index].createdBy===actor.username,'Support ticket not found',404);return {tickets,index,ticket:tickets[index]};}
  replySupportTicket(id,body,actor){const {tickets,index,ticket}=this.supportTicket(id,actor),message=cleanText(body.message,5000),photo=supportPhoto(body.photo);check(message||photo,'Add a reply or attachment');const now=new Date().toISOString(),reply={id:crypto.randomUUID(),author:actor.username,isAdmin:actor.isAdmin,body:message,photo,createdAt:now};tickets[index]={...ticket,status:ticket.status==='Resolved'?'In Progress':ticket.status,updatedAt:now,messages:[...(ticket.messages||[]),reply]};this.ctx.storage.transactionSync(()=>{this.write('support-tickets',tickets);this.notify(actor.isAdmin?[ticket.createdBy]:this.adminUsernames().filter(username=>username!==actor.username),{title:'Support ticket reply',message:ticket.subject,kind:'support',link:'support'});this.audit(actor.username,'support-ticket-replied',{ticketId:id});});return ok({ok:true,ticket:tickets[index],tickets:this.supportTicketsFor(actor)});}
  updateSupportStatus(id,body,actor){check(actor.isAdmin,'Admin access required',403);const {tickets,index,ticket}=this.supportTicket(id,actor),status=cleanText(body.status,30);check(['Open','In Progress','Resolved'].includes(status),'Invalid support status');tickets[index]={...ticket,status,updatedAt:new Date().toISOString()};this.ctx.storage.transactionSync(()=>{this.write('support-tickets',tickets);this.notify([ticket.createdBy],{title:`Support ticket ${status.toLowerCase()}`,message:ticket.subject,kind:'support',link:'support'});this.audit(actor.username,'support-ticket-status',{ticketId:id,status});});return ok({ok:true,ticket:tickets[index],tickets:this.supportTicketsFor(actor)});}
  qaSettings(){const saved=this.read('qa-settings',{}),definitions=(kind,name)=>{const fallback=QA_CHECKLISTS[kind],source=Array.isArray(saved[name])?saved[name]:fallback,result=[],seen=new Set();for(const item of source){const key=cleanText(Array.isArray(item)?item[0]:item?.key,50).toLowerCase(),label=cleanText(Array.isArray(item)?item[1]:item?.label,120);if(/^[a-z0-9][a-z0-9_-]{0,49}$/.test(key)&&label&&!seen.has(key)){seen.add(key);result.push([key,label]);}}return result.length?result:fallback;},panelChecks=definitions('panel','panelChecks'),metalworkChecks=definitions('metalwork','metalworkChecks'),active=(items,name)=>{const available=new Set(items.map(([key])=>key)),selected=Array.isArray(saved[name])?[...new Set(saved[name].map(key=>cleanText(key,50).toLowerCase()).filter(key=>available.has(key)))]:items.map(([key])=>key);return selected.length?selected:items.map(([key])=>key);};return {enabledFrom:saved.enabledFrom||new Date().toISOString(),requireQaPhoto:saved.requireQaPhoto===true,requireFailurePhoto:saved.requireFailurePhoto!==false,preventSelfApproval:saved.preventSelfApproval!==false,allowSelfApprovalOverride:saved.allowSelfApprovalOverride!==false,requireResolvedOrderForDispatch:saved.requireResolvedOrderForDispatch!==false,allowDispatchOverride:saved.allowDispatchOverride!==false,requireDispatchPhoto:saved.requireDispatchPhoto===true,requireDestination:saved.requireDestination!==false,requireTransport:saved.requireTransport!==false,requireDriver:saved.requireDriver!==false,panelChecks,metalworkChecks,activePanelChecks:active(panelChecks,'activePanelChecks'),activeMetalworkChecks:active(metalworkChecks,'activeMetalworkChecks')};}
  qaChecklists(settings=this.qaSettings()){return {panel:settings.panelChecks.filter(([key])=>settings.activePanelChecks.includes(key)),metalwork:settings.metalworkChecks.filter(([key])=>settings.activeMetalworkChecks.includes(key))};}
  qaSettingsFor(actor){check(actor.isAdmin,'Admin access required',403);const settings=this.qaSettings();return {settings,availableChecklists:{panel:settings.panelChecks,metalwork:settings.metalworkChecks}};}
  updateQaSettings(body,actor){check(actor.isAdmin,'Admin access required',403);const current=this.qaSettings(),definitions=(kind,value)=>{check(Array.isArray(value)&&value.length>0&&value.length<=50,`Add between 1 and 50 ${kind} checks`);const seen=new Set(),items=value.map(item=>{const key=cleanText(Array.isArray(item)?item[0]:item?.key,50).toLowerCase(),label=cleanText(Array.isArray(item)?item[1]:item?.label,120);check(/^[a-z0-9][a-z0-9_-]{0,49}$/.test(key)&&label&&!seen.has(key),`Invalid or duplicate ${kind} check`);seen.add(key);return [key,label];});return items;},active=(kind,value,items)=>{const available=new Set(items.map(([key])=>key));check(Array.isArray(value)&&value.length>0,`Select at least one ${kind} check`);const selected=[...new Set(value.map(key=>cleanText(key,50).toLowerCase()).filter(key=>available.has(key)))];check(selected.length===value.length,`Invalid ${kind} checklist`);return selected;},booleanFields=['requireQaPhoto','requireFailurePhoto','preventSelfApproval','allowSelfApprovalOverride','requireResolvedOrderForDispatch','allowDispatchOverride','requireDispatchPhoto','requireDestination','requireTransport','requireDriver'];for(const key of booleanFields)check(typeof body[key]==='boolean',`Invalid ${key} setting`);const panelChecks=definitions('panel',body.panelChecks),metalworkChecks=definitions('metalwork',body.metalworkChecks),settings={enabledFrom:current.enabledFrom,...Object.fromEntries(booleanFields.map(key=>[key,body[key]])),panelChecks,metalworkChecks,activePanelChecks:active('panel',body.activePanelChecks,panelChecks),activeMetalworkChecks:active('metalwork',body.activeMetalworkChecks,metalworkChecks)};this.ctx.storage.transactionSync(()=>{this.write('qa-settings',settings);this.audit(actor.username,'qa-settings',{...settings,enabledFrom:undefined});});return ok({ok:true,settings,availableChecklists:{panel:panelChecks,metalwork:metalworkChecks}});}
  qaPanelItem(panel,record){return {id:panel.id,kind:'panel',project:cleanText(panel.jobReference,120),orderNumber:cleanText(panel.orderNumber,100),sheetNumber:cleanText(panel.sheetNumber,100),reference:cleanText(panel.panelNumber,120),quantity:1,material:cleanText(panel.material||panel.stockMaterial,120),colour:cleanText(panel.color||panel.colour||panel.stockColor,120),thickness:Number(panel.thickness||panel.stockThickness)||0,width:Number(panel.panelWidth)||0,height:Number(panel.panelHeight)||0,cutBy:cleanText(panel.completedBy,100)||'Unknown',cutAt:panel.completedAt||null,isRemake:panel.isRemake===true,remakeReason:cleanText(panel.remakeReason,500),status:record?.status||'awaiting',latest:record?.latest||null,history:record?.history||[],remakeOf:record?.remakeOf||null,replacementId:record?.replacementId||null,replacementStatus:record?.replacementStatus||null,resolvedSeparatelyAt:record?.resolvedSeparatelyAt||null,resolvedSeparatelyBy:record?.resolvedSeparatelyBy||null,resolvedSeparatelyReason:record?.resolvedSeparatelyReason||''};}
  qaItemsFor(actor){this.requireTask(actor,'factory.qa');const settings=this.qaSettings(),storedRecords=this.read('qa-checks',[]),migration=migrateQaRecords(storedRecords),records=migration.records,migrationCount=migration.count,byId=new Map(records.filter(record=>record.kind==='panel').map(record=>[record.id,record])),panels=this.read('app:cncPanels',[]),panelItems=panels.filter(panel=>panel.status==='completed'&&panel.completedAt&&String(panel.completedAt)>=settings.enabledFrom).map(panel=>this.qaPanelItem(panel,byId.get(panel.id))),itemMap=new Map(panelItems.map(item=>[item.id,item]));for(const item of panelItems){if(item.remakeOf)item.remakeOfReference=itemMap.get(item.remakeOf)?.reference||'';if(item.replacementId)item.replacementReference=itemMap.get(item.replacementId)?.reference||'';}const metalwork=records.filter(record=>record.kind==='metalwork').map(record=>({...record,status:record.status||'awaiting'})),items=[...panelItems,...metalwork],dispatches=this.read('qa-dispatches',[]).slice().sort((a,b)=>String(b.dispatchedAt).localeCompare(String(a.dispatchedAt)));for(const item of items){item.dispatchedQuantity=dispatches.reduce((sum,dispatch)=>sum+(dispatch.lines||[]).filter(line=>line.id===item.id).reduce((lineSum,line)=>lineSum+(Number(line.quantity)||0),0),0);item.remainingQuantity=Math.max(0,(Number(item.quantity)||1)-item.dispatchedQuantity);item.dispatchStatus=item.remainingQuantity<=0?'dispatched':item.dispatchedQuantity>0?'partial':'ready';}if(migrationCount)this.ctx.storage.transactionSync(()=>{this.write('qa-checks',records);this.audit('system','qa-rework-migrated',{count:migrationCount});});const preQaCount=panels.filter(panel=>panel.status==='completed'&&(!panel.completedAt||String(panel.completedAt)<settings.enabledFrom)).length;return {settings,checklists:this.qaChecklists(settings),items:items.sort((a,b)=>String(b.cutAt||b.createdAt||'').localeCompare(String(a.cutAt||a.createdAt||''))),dispatches,preQaCount};}
  createQaMetalwork(body,actor){this.requireTask(actor,'factory.qa');const project=cleanText(body.project,120),orderNumber=cleanText(body.orderNumber,100),reference=cleanText(body.reference,120),quantity=Number(body.quantity),fabricatedBy=cleanText(body.fabricatedBy,100);check(project&&orderNumber&&reference,'Project, order number and item reference are required');check(Number.isInteger(quantity)&&quantity>0&&quantity<=10000,'Enter a valid quantity');check(fabricatedBy,'Fabricated by is required');const now=new Date().toISOString(),record={id:crypto.randomUUID(),kind:'metalwork',project,orderNumber,sheetNumber:'',reference,quantity,fabricatedBy,createdBy:actor.username,createdAt:now,status:'awaiting',latest:null,history:[]},records=this.read('qa-checks',[]);records.unshift(record);this.ctx.storage.transactionSync(()=>{this.write('qa-checks',records);this.audit(actor.username,'qa-metalwork-added',{qaId:record.id,project,orderNumber,reference,quantity,fabricatedBy});});return ok({ok:true,item:record,...this.qaItemsFor(actor)},201);}
  resolveQaRecut(body,actor){
    check(actor.isAdmin,'Admin access required',403);
    const id=cleanText(body.id,100),reason=cleanText(body.reason,500);
    check(id&&reason,'Recut item and resolution reason are required');
    const records=this.read('qa-checks',[]),index=records.findIndex(record=>record.id===id&&record.kind==='panel');
    check(index>=0&&records[index].status==='recut','Active panel recut not found',404);
    const panel=this.read('app:cncPanels',[]).find(value=>value.id===id&&value.status==='completed');
    check(panel,'Completed CNC panel not found',404);
    const now=new Date().toISOString(),record={...records[index],status:'replaced',replacementStatus:'resolved-separately',resolvedSeparatelyAt:now,resolvedSeparatelyBy:actor.username,resolvedSeparatelyReason:reason};
    records[index]=record;
    this.ctx.storage.transactionSync(()=>{this.write('qa-checks',records);this.audit(actor.username,'qa-recut-resolved-separately',{qaId:id,project:cleanText(panel.jobReference,120),orderNumber:cleanText(panel.orderNumber,100),sheetNumber:cleanText(panel.sheetNumber,100),reference:cleanText(panel.panelNumber,120),reason});});
  ...
  createQaDispatch(body,actor){this.requireTask(actor,'factory.qa.dispatch');const settings=this.qaSettings(),project=cleanText(body.project,120),orderNumber=cleanText(body.orderNumber,100),destination=cleanText(body.destination,300),transport=cleanText(body.transport,160),driver=cleanText(body.driver,160),notes=cleanText(body.notes,5000),photo=supportPhoto(body.photo),overrideReason=cleanText(body.overrideReason,500),requested=Array.isArray(body.lines)?body.lines:[];check(project&&orderNumber,'Project and order number are required');if(settings.requireDestination)check(destination,'Destination is required');if(settings.requireTransport)check(transport,'Vehicle or transport company is required');if(settings.requireDriver)check(driver,'Driver is required');if(settings.requireDispatchPhoto)check(photo,'Add a dispatch photo');check(requested.length>0&&requested.length<=500,'Select at least one item for dispatch');const view=this.qaItemsFor(actor),orderItems=view.items.filter(item=>item.project.toLowerCase()===project.toLowerCase()&&item.orderNumber.toLowerCase()===orderNumber.toLowerCase()),byId=new Map(orderItems.map(item=>[item.id,item])),seen=new Set(),lines=[];check(orderItems.length,'No QA items were found for this order',404);const unresolved=orderItems.filter(item=>['awaiting','recut'].includes(item.status));if(unresolved.length&&settings.requireResolvedOrderForDispatch)check(settings.allowDispatchOverride&&actor.isAdmin&&overrideReason,'Every item in this order must pass QA before dispatch. An administrator override requires a reason',409);for(const input of requested){const id=cleanText(input?.id,100),item=byId.get(id),quantity=Number(input?.quantity);check(item&&!seen.has(id),'Invalid or duplicate dispatch item');seen.add(id);check(item.status!=='replaced','Dispatch the approved replacement instead of the failed original');check(item.status!=='recut','Failed items must be recut or corrected before dispatch',409);if(item.status!=='approved')check(settings.allowDispatchOverride&&actor.isAdmin&&overrideReason,'Only QA-approved items may be dispatched',409);check(Number.isInteger(quantity)&&quantity>0&&quantity<=item.remainingQuantity,'Dispatch quantity exceeds the approved quantity');lines.push({id:item.id,kind:item.kind,project:item.project,orderNumber:item.orderNumber,sheetNumber:item.sheetNumber||'',reference:item.reference,quantity,producer:item.kind==='panel'?item.cutBy:item.fabricatedBy,qaCheckedBy:item.latest?.checkedBy||'',qaCheckedAt:item.latest?.checkedAt||''});}const now=new Date().toISOString(),record={id:crypto.randomUUID(),project,orderNumber,destination,transport,driver,notes,photo,overrideReason,lines,dispatchedBy:actor.username,dispatchedAt:now},dispatches=this.read('qa-dispatches',[]);dispatches.unshift(record);this.ctx.storage.transactionSync(()=>{this.write('qa-dispatches',dispatches);this.audit(actor.username,'qa-dispatch-load',{dispatchId:record.id,project,orderNumber,itemCount:lines.length,quantity:lines.reduce((sum,line)=>sum+line.quantity,0),override:!!overrideReason});});return ok({ok:true,dispatch:record,...this.qaItemsFor(actor)},201);}
  submitQaCheck(body,actor){
    this.requireTask(actor,'factory.qa');
    const settings=this.qaSettings(),id=cleanText(body.id,100),kind=cleanText(body.kind,20),definitions=this.qaChecklists(settings)[kind];
    check(id&&definitions,'Invalid QA item');
    const records=this.read('qa-checks',[]),index=records.findIndex(record=>record.id===id),existing=index>=0?records[index]:null,cncPanels=this.read('app:cncPanels',[]);
    let source,panel;
    if(kind==='panel'){
      panel=cncPanels.find(value=>value.id===id&&value.status==='completed');
      check(panel,'Completed CNC panel not found',404);
      check(panel.completedAt&&String(panel.completedAt)>=this.qaSettings().enabledFrom,'This panel was completed before QA checks went live',409);
      check(existing?.status!=='recut','Failed panels require a new CNC recut',409);
      source=this.qaPanelItem(panel,existing);
    }else{
      check(existing&&existing.kind==='metalwork','Metalwork item not found',404);
      source=existing;
    }
    if(existing?.status==='approved'||existing?.status==='replaced')this.requireTask(actor,'factory.qa.manage');
    const results={};
    for(const [key] of definitions){const value=cleanText(body.results?.[key],10);check(['pass','fail','na'].includes(value),'Complete every QA check');results[key]=value;}
    const failed=Object.values(results).includes('fail'),status=failed?'recut':'approved',notes=cleanText(body.notes,5000),photo=supportPhoto(body.photo),producer=kind==='panel'?source.cutBy:source.fabricatedBy,overrideReason=cleanText(body.overrideReason,500);
    if(settings.requireQaPhoto)check(photo,'Add a QA photo');
    if(failed){check(notes,'Add notes for failed QA checks');if(settings.requireFailurePhoto)check(photo,'Add a photo for failed QA checks');}
    if(!failed&&settings.preventSelfApproval&&normalizeUsername(producer)===actor.username)check(settings.allowSelfApprovalOverride&&actor.isAdmin&&overrideReason,settings.allowSelfApprovalOverride?'You cannot approve work you produced. An administrator override requires a reason':'Work must be approved by another person',403);
    const now=new Date().toISOString(),attempt={id:crypto.randomUUID(),status,results,notes,photo,checkedBy:actor.username,checkedAt:now,...(overrideReason?{overrideReason}: {})},record=kind==='panel'?{id,kind:'panel',status,latest:attempt,history:[...(existing?.history||[]),attempt],...(existing?.remakeOf?{remakeOf:existing.remakeOf}:{})}:{...existing,status,latest:attempt,history:[...(existing.history||[]),attempt]};
    if(kind==='panel'&&panel.isRemake===true&&!record.remakeOf){
      const key=value=>JSON.stringify([cleanText(value.jobReference,120).toLowerCase(),cleanText(value.orderNumber,100).toLowerCase(),cleanText(value.panelNumber,120).toLowerCase()]),targetKey=key(panel),panelMap=new Map(cncPanels.map(value=>[value.id,value])),candidate=records.filter(value=>value.kind==='panel'&&value.id!==id&&value.status==='recut').map(value=>({record:value,panel:panelMap.get(value.id)})).filter(value=>value.panel&&key(value.panel)===targetKey&&String(value.record.latest?.checkedAt||'')<String(panel.completedAt||'')).sort((a,b)=>String(b.record.latest?.checkedAt||'').localeCompare(String(a.record.latest?.checkedAt||'')))[0];
  ...
  mutate(body,actor) {
    ...
    // PDF reuploads and manual admin edits keep record IDs and use the same
    // optimistic, atomic mutation as their audit entry. Validate the complete
    // saved schedule so moving a panel cannot create duplicates or overbooking.
    const revisedPanels=normalized.filter(c=>c.field==='cncPanels'&&c.before?.status==='pending'&&c.after?.status==='pending'&&JSON.stringify(c.after)!==JSON.stringify(c.before));
    if(revisedPanels.length) {
      ...
      const touchedSheets=new Set(revisedPanels.flatMap(c=>[sheetKey(c.before),sheetKey(c.after)])),sheetStocks=new Map(),reservations=new Map(),keys=new Set();
      for(const panel of currentMap('cncPanels').values()) {
        const sheet=sheetKey(panel);
        if(touchedSheets.has(sheet)) {
          check(panel.status==='pending','A completed CNC sheet cannot be replaced, extended or edited',409);
          const key=cncDuplicateKey(panel);
          check(!keys.has(key),'Duplicate CNC panel in schedule',409);keys.add(key);
      ...
      for(const c of revisedPanels) {
        ...
        check((reservations.get(stockKey(panel))?.size||0)<=stock.qty,'Not enough unreserved stock for this CNC schedule',409);
      }
    }
    ...
    if(scheduled.length){
      ...
      this.notify(this.adminUsernames(),{title:'CNC sheets scheduled',message:`${actor.username} scheduled ${sheets.size} sheet${sheets.size===1?'':'s'} (${scheduled.length} panel${scheduled.length===1?'':'s'})${context}.`,kind:'cnc',priority:'important',link:'cnc'});
    }
  ...
  async handle(path,method,body,token,ip) {
    try {
      ...
      if(path==='/notifications/clear' && method==='POST') return this.clearNotifications(actor);
      if(path==='/notification-preferences' && method==='GET') return ok({ok:true,preferences:this.notificationPreferences(actor.username)});
      if(path==='/notification-preferences' && method==='POST') return this.updateNotificationPreferences(body,actor);
      if(path==='/live-ticket' && method==='GET') return this.createLiveTicket(actor);
      if(path==='/push/config' && method==='GET') return ok({ok:true,publicKey:this.env.VAPID_PUBLIC_KEY||'',enabled:!!(this.env.VAPID_PUBLIC_KEY&&this.env.VAPID_PRIVATE_KEY)});
      if(path==='/push/subscribe' && method==='POST') return this.savePushSubscription(body,actor);
      if(path==='/push/unsubscribe' && method==='POST') return this.removePushSubscription(body,actor);
      if(path==='/support' && method==='GET') return ok({ok:true,tickets:this.supportTicketsFor(actor)});
      if(path==='/support' && method==='POST') return this.createSupportTicket(body,actor);
      const supportReplyPath=path.match(/^\/support\/([a-zA-Z0-9-]{16,100})\/reply$/);
      ...
      if(path==='/qa/settings' && method==='POST') return this.updateQaSettings(body,actor);
      if(path==='/qa/check' && method==='POST') return this.submitQaCheck(body,actor);
  ...
  cncSettings() {return normalizeCncSettings(this.read('cnc-settings',{}));}
  updateCncSettings(body,actor) {const fields=['minimumOffcutSizeMm','wasteGreenMax','wasteYellowMax','cutEdgeAllowanceMm'],values=Object.fromEntries(fields.map(key=>[key,Number(body[key])]));check(Number.isInteger(values.minimumOffcutSizeMm)&&values.minimumOffcutSizeMm>=1&&values.minimumOffcutSizeMm<=10000,'Minimum off-cut size must be a whole number between 1 and 10000 mm');check(Number.isInteger(values.cutEdgeAllowanceMm)&&values.cutEdgeAllowanceMm>=0&&values.cutEdgeAllowanceMm<=1000,'Cut-edge allowance must be a whole number between 0 and 1000 mm');check([values.wasteGreenMax,values.wasteYellowMax].every(value=>Number.isInteger(value)&&value>=0&&value<=100),'Wastage percentages must be whole numbers between 0 and 100');check(values.wasteGreenMax<values.wasteYellowMax,'The yellow wastage limit must be greater than the green limit');this.ctx.storage.transactionSync(()=>{this.write('cnc-settings',values);this.audit(actor.username,'cnc-settings',values);});return ok({ok:true,settings:values});}
  ...
  createScheduleEntry(body,actor) {const now=new Date().toISOString(),entry={id:crypto.randomUUID(),...this.scheduleValue(body.entry||body),createdAt:now,createdBy:actor.username,updatedAt:now,updatedBy:actor.username},entries=this.read('schedule',[]);entries.push(entry);this.ctx.storage.transactionSync(()=>{this.write('schedule',entries);this.notify([entry.assignedUsername],{title:'New schedule entry',message:`${entry.date} · ${entry.title||entry.project}`,kind:'schedule',priority:'important',link:'schedule'});this.audit(actor.username,'schedule-created',{scheduleId:entry.id,project:entry.project,date:entry.date,type:entry.scheduleType});});return ok({ok:true,entry,entries:this.scheduleEntriesFor(actor)},201);}
  updateScheduleEntry(id,body,actor) {const entries=this.read('schedule',[]),index=entries.findIndex(value=>value.id===id);check(index>=0,'Schedule entry not found',404);const previous=entries[index];entries[index]={...this.scheduleValue(body.entry||body,previous),updatedAt:new Date().toISOString(),updatedBy:actor.username};this.ctx.storage.transactionSync(()=>{this.write('schedule',entries);this.notify([previous.assignedUsername,entries[index].assignedUsername],{title:'Schedule updated',message:`${entries[index].date} · ${entries[index].title||entries[index].project}`,kind:'schedule',priority:'important',link:'schedule'});this.audit(actor.username,'schedule-updated',{scheduleId:id});});return ok({ok:true,entry:entries[index],entries:this.scheduleEntriesFor(actor)});}
  deleteScheduleEntry(id,actor) {const entries=this.read('schedule',[]),index=entries.findIndex(value=>value.id===id);check(index>=0,'Schedule entry not found',404);const [entry]=entries.splice(index,1);this.ctx.storage.transactionSync(()=>{this.write('schedule',entries);this.notify([entry.assignedUsername],{title:'Schedule entry cancelled',message:`${entry.date} · ${entry.title||entry.project}`,kind:'schedule',priority:'important',link:'schedule'});this.audit(actor.username,'schedule-deleted',{scheduleId:id,project:entry.project,date:entry.date});});return ok({ok:true,entries:this.scheduleEntriesFor(actor)});}

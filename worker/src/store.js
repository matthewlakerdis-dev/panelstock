...
export class InventoryStore extends DurableObject {
  ...
  updateCncSettings(body,actor) {const fields=['minimumOffcutSizeMm','wasteGreenMax','wasteYellowMax','wasteOrangeMax','cutEdgeAllowanceMm'],values=Object.fromEntries(fields.map(key=>[key,Number(body[key])]));check(Number.isInteger(values.minimumOffcutSizeMm)&&values.minimumOffcutSizeMm>=1&&values.minimumOffcutSizeMm<=10000,'Minimum off-cut size must be a whole number between 1 and 10000 mm');check(Number.isInteger(values.cutEdgeAllowanceMm)&&values.cutEdgeAllowanceMm>=0&&values.cutEdgeAllowanceMm<=1000,'Cut-edge allowance must be a whole number between 0 and 1000 mm');check([values.wasteGreenMax,values.wasteYellowMax,values.wasteOrangeMax].every(value=>Number.isInteger(value)&&value>=0&&value<=100),'Wastage percentages must be whole numbers between 0 and 100');check(values.wasteGreenMax<values.wasteYellowMax&&values.wasteYellowMax<values.wasteOrangeMax,'Wastage percentages must increase from green to yellow to orange');this.ctx.storage.transactionSync(()=>{this.write('cnc-settings',values);this.audit(actor.username,'cnc-settings',values);});return ok({ok:true,settings:values});}

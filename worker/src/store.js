  ...
  async handle(path,method,body,token,ip) {
    try {
      ...
      if(path==='/admin/create-user') {
        const target=normalizeUsername(body.targetUsername),displayName=String(body.displayName||'').trim().replace(/\s+/g,' ').slice(0,100),temporaryPin=String(body.temporaryPin||''),makeAdmin=body.makeAdmin===true,roleIds=[...new Set(Array.isArray(body.roleIds)?body.roleIds:(body.roleId?[body.roleId]:[]))].filter(Boolean);
        ...
        current[target]={password,displayName,title:'',location:'',active:true,isAdmin:makeAdmin,mustChangePin:true,updatedAt:new Date().toISOString()};this.ctx.storage.transactionSync(()=>{this.write('users',current);this.syncAccessUser(target,current[target]);this.sql.exec('UPDATE access_users SET role_id=? WHERE username=?',roleIds[0]||null,target);for(const roleId of roleIds)this.sql.exec('INSERT INTO user_roles(username,role_id) VALUES(?,?)',target,roleId);this.audit(actor.username,'admin/create-user',{target,isAdmin:makeAdmin,roleIds});});return ok({ok:true,user:{username:target,displayName,title:'',location:'',active:true,isAdmin:makeAdmin,roleIds,roleId:roleIds[0]||null,taskAccess:this.taskAccess(target,makeAdmin)}},201);
      }
      if(path==='/admin/update-user') {
        const target=normalizeUsername(body.targetUsername),displayName=String(body.displayName||'').trim().replace(/\s+/g,' ').slice(0,100),title=String(body.title||'').trim().slice(0,100),location=String(body.location||'').trim().slice(0,160),active=body.active===true,isAdmin=body.isAdmin===true,roleIds=[...new Set(Array.isArray(body.roleIds)?body.roleIds:(body.roleId?[body.roleId]:[]))].filter(Boolean);check(Object.hasOwn(users,target),'User not found',404);check(displayName,'Display name is required');check(roleIds.length<=20&&roleIds.every(roleId=>this.sql.exec('SELECT id FROM access_roles WHERE id=?',roleId).toArray()[0]),'Role not found',404);check(active||target!==actor.username,'You cannot deactivate your own account');if(users[target].isAdmin&&(!isAdmin||!active))check(Object.values(users).filter(user=>user.isAdmin&&user.active!==false).length>1,'Cannot deactivate or demote the last active admin');
        const previous=users[target],previousRoles=this.userRoleIds(target),revoke=previous.active!==active||!!previous.isAdmin!==isAdmin||JSON.stringify(previousRoles)!==JSON.stringify([...roleIds].sort());users[target]={...previous,displayName,title,location,active,isAdmin,updatedAt:new Date().toISOString()};this.ctx.storage.transactionSync(()=>{this.write('users',users);this.sql.exec('UPDATE access_users SET display_name=?,title=?,location=?,active=?,is_admin=?,role_id=?,updated_at=? WHERE username=?',displayName,title,location,active?1:0,isAdmin?1:0,roleIds[0]||null,new Date().toISOString(),target);this.sql.exec('DELETE FROM user_roles WHERE username=?',target);for(const roleId of roleIds)this.sql.exec('INSERT INTO user_roles(username,role_id) VALUES(?,?)',target,roleId);if(revoke)this.sql.exec('DELETE FROM sessions WHERE username=?',target);this.audit(actor.username,'admin/update-user',{target,active,isAdmin,roleIds});});return ok({ok:true,user:{username:target,displayName,title,location,active,isAdmin,roleIds,roleId:roleIds[0]||null,taskAccess:this.taskAccess(target,isAdmin)}});
      }
      if(path==='/admin/rename-user') {
        ...
        const roleIds=this.userRoleIds(next);return ok({ok:true,user:{username:next,displayName:renamed.displayName||next,title:renamed.title||'',location:renamed.location||'',active:renamed.active!==false,isAdmin:!!renamed.isAdmin,roleIds,roleId:roleIds[0]||null,taskAccess:this.taskAccess(next,!!renamed.isAdmin)},aliasExpiresAt});
      }

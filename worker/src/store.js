...
export class InventoryStore extends DurableObject {
  ...
  notify(recipients,{title,message,kind='general',priority='normal',link=''}){const users=this.read('users',{}),valid=[...new Set((Array.isArray(recipients)?recipients:[recipients]).map(normalizeUsername).filter(username=>users[username]?.active!==false))];if(!valid.length)return;const now=new Date().toISOString(),items=this.read('notifications',[]),created=[];for(const recipient of valid){const item={id:crypto.randomUUID(),recipient,title:cleanText(title,140),message:cleanText(message,500),kind:cleanText(kind,40),priority:['normal','important','urgent'].includes(priority)?priority:'normal',link:cleanText(link,40),createdAt:now,readBy:[]};items.unshift(item);created.push(item);}this.write('notifications',items.slice(0,2000));if(this.env.VAPID_PUBLIC_KEY&&this.env.VAPID_PRIVATE_KEY)this.ctx.waitUntil(Promise.all(created.map(item=>this.sendPush(item))));}

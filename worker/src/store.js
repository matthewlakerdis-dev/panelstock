...
export class InventoryStore extends DurableObject {
  ...
  createQaDispatch(body,actor){this.requireTask(actor,'factory.qa.dispatch');const settings=this.qaSettings(),project=cleanText(body.project,120),orderNumber=cleanText(body.orderNumber,100),destination=cleanText(body.destination,300),transport=cleanText(body.transport,160),driver=cleanText(body.driver,160),notes=cleanText(body.notes,5000),photo=supportPhoto(body.photo),overrideReason=cleanText(body.overrideReason,500),requested=Array.isArray(body.lines)?body.lines:[];check(project&&orderNumber,'Project and order number are required');if(settings.requireDestination)check(destination,'Destination is required');if(settings.requireTransport)check(transport,'Vehicle or transport company is required');if(settings.requireDriver)check(driver,'Driver is required');if(settings.requireDispatchPhoto)check(photo,'Add a dispatch photo');check(requested.length>0&&requested.length<=500,'Select at least one item for dispatch');const view=this.qaItemsFor(actor),orderItems=view.items.filter(item=>item.project.toLowerCase()===project.toLowerCase()&&item.orderNumber.toLowerCase()===orderNumber.toLowerCase()),byId=new Map(orderItems.map(item=>[item.id,item])),seen=new Set(),lines=[];check(orderItems.length,'No QA items were found for this order',404);const unresolved=orderItems.filter(item=>['awaiting','rework'].includes(item.status));if(unresolved.length&&settings.requireResolvedOrderForDispatch)check(settings.allowDispatchOverride&&actor.isAdmin&&overrideReason,'Every item in this order must pass QA before dispatch. An administrator override requires a reason',409);for(const input of requested){const id=cleanText(input?.id,100),item=byId.get(id),quantity=Number(input?.quantity);check(item&&!seen.has(id),'Invalid or duplicate dispatch item');seen.add(id);check(item.status!=='replaced','Dispatch the approved replacement instead of the failed original');if(item.status!=='approved')check(settings.allowDispatchOverride&&actor.isAdmin&&overrideReason,'Only QA-approved items may be dispatched',409);check(Number.isInteger(quantity)&&quantity>0&&quantity<=item.remainingQuantity,'Dispatch quantity exceeds the approved quantity');lines.push({id:item.id,kind:item.kind,project:item.project,orderNumber:item.orderNumber,sheetNumber:item.sheetNumber||'',reference:item.reference,quantity,producer:item.kind==='panel'?item.cutBy:item.fabricatedBy,qaCheckedBy:item.latest?.checkedBy||'',qaCheckedAt:item.latest?.checkedAt||''});}const now=new Date().toISOString(),record={id:crypto.randomUUID(),project,orderNumber,destination,transport,driver,notes,photo,overrideReason,lines,dispatchedBy:actor.username,dispatchedAt:now},dispatches=this.read('qa-dispatches',[]);dispatches.unshift(record);this.ctx.storage.transactionSync(()=>{this.write('qa-dispatches',dispatches);this.audit(actor.username,'qa-dispatch-load',{dispatchId:record.id,project,orderNumber,itemCount:lines.length,quantity:lines.reduce((sum,line)=>sum+line.quantity,0),override:!!overrideReason});});return ok({ok:true,dispatch:record,...this.qaItemsFor(actor)},201);}
  submitQaCheck(body,actor){this.requireTask(actor,'factory.qa');const settings=this.qaSettings(),id=cleanText(body.id,100),kind=cleanText(body.kind,20),definitions=this.qaChecklists(settings)[kind];check(id&&definitions,'Invalid QA item');const records=this.read('qa-checks',[]),index=records.findIndex(record=>record.id===id),existing=index>=0?records[index]:null,cncPanels=this.read('app:cncPanels',[]);let source,panel;if(kind==='panel'){panel=cncPanels.find(value=>value.id===id&&value.status==='completed');check(panel,'Completed CNC panel not found',404);check(panel.completedAt&&String(panel.completedAt)>=this.qaSettings().enabledFrom,'This panel was completed before QA checks went live',409);source=this.qaPanelItem(panel,existing);}else{check(existing&&existing.kind==='metalwork','Metalwork item not found',404);source=existing;}if(existing?.status==='approved'||existing?.status==='replaced')this.requireTask(actor,'factory.qa.manage');const results={};for(const [key] of definitions){const value=cleanText(body.results?.[key],10);check(['pass','fail','na'].includes(value),'Complete every QA check');results[key]=value;}const failed=Object.values(results).includes('fail'),status=failed?'rework':'approved',notes=cleanText(body.notes,5000),photo=supportPhoto(body.photo),producer=kind==='panel'?source.cutBy:source.fabricatedBy,overrideReason=cleanText(body.overrideReason,500);if(settings.requireQaPhoto)check(photo,'Add a QA photo');if(failed){check(notes,'Add notes for failed QA checks');if(settings.requireFailurePhoto)check(photo,'Add a photo for failed QA checks');}if(!failed&&settings.preventSelfApproval&&normalizeUsername(producer)===actor.username){check(settings.allowSelfApprovalOverride&&actor.isAdmin&&overrideReason,settings.allowSelfApprovalOverride?'You cannot approve work you produced. An administrator override requires a reason':'Work must be approved by another person',403);}const now=new Date().toISOString(),attempt={id:crypto.randomUUID(),status,results,notes,photo,checkedBy:actor.username,checkedAt:now,...(overrideReason?{overrideReason}: {})},record=kind==='panel'?{id,kind:'panel',status,latest:attempt,history:[...(existing?.history||[]),attempt],...(existing?.remakeOf?{remakeOf:existing.remakeOf}:{})}:{...existing,status,latest:attempt,history:[...(existing.history||[]),attempt]};if(kind==='panel'&&panel.isRemake===true&&!record.remakeOf){const key=value=>JSON.stringify([cleanText(value.jobReference,120).toLowerCase(),cleanText(value.orderNumber,100).toLowerCase(),cleanText(value.panelNumber,120).toLowerCase()]),targetKey=key(panel),panelMap=new Map(cncPanels.map(value=>[value.id,value])),candidate=records.filter(value=>value.kind==='panel'&&value.id!==id&&value.status==='rework').map(value=>({record:value,panel:panelMap.get(value.id)})).filter(value=>value.panel&&key(value.panel)===targetKey&&String(value.record.latest?.checkedAt||'')<String(panel.completedAt||'')).sort((a,b)=>String(b.record.latest?.checkedAt||'').localeCompare(String(a.record.latest?.checkedAt||'')))[0];if(candidate){record.remakeOf=candidate.record.id;const originalIndex=records.findIndex(value=>value.id===candidate.record.id);records[originalIndex]={...candidate.record,...(!failed?{status:'replaced'}:{}),replacementId:id,replacementStatus:status,replacedAt:!failed?now:null};}}if(record.remakeOf){const originalIndex=records.findIndex(value=>value.id===record.remakeOf);if(originalIndex>=0)records[originalIndex]={...records[originalIndex],...(!failed?{status:'replaced'}:{status:'rework'}),replacementId:id,replacementStatus:status,replacedAt:!failed?now:null};}if(index>=0)records[index]=record;else records.unshift(record);this.ctx.storage.transactionSync(()=>{this.write('qa-checks',records);this.audit(actor.username,status==='approved'?'qa-approved':'qa-rework-required',{qaId:id,kind,project:source.project,orderNumber:source.orderNumber,reference:source.reference,producer,override:!!overrideReason,remakeOf:record.remakeOf||null});});const view=this.qaItemsFor(actor);return ok({ok:true,item:view.items.find(value=>value.id===id)||(kind==='panel'?this.qaPanelItem(panel,record):record),...view});}

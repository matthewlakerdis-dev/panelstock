...
export class InventoryStore extends DurableObject {
  ...
  async handle(path,method,body,token,ip) {
    try {
      ...
      if(path==='/schedule' && method==='GET') {this.requireTask(actor,'schedule.view');return ok({ok:true,entries:this.scheduleEntries(),projects:this.ensureProjectRecords(),people:this.schedulePeople(),viewer:actor.username});}
  ...
  schedulePeople() {return this.sql.exec('SELECT username,display_name AS displayName FROM access_users WHERE active=1 ORDER BY display_name,username').toArray().map(value=>({username:value.username,displayName:value.displayName||value.username}));}
  scheduleValue(input,existing={}) {
    const clean=value=>String(value??'').trim(),projects=this.ensureProjectRecords(),selected=projects.find(value=>value.id===input.projectId)||projects.find(value=>this.orderProjectKey(value.name)===this.orderProjectKey(input.project)),people=this.schedulePeople(),assignedUsername=normalizeUsername(input.assignedUsername||existing.assignedUsername||''),person=people.find(value=>value.username===assignedUsername),date=clean(input.date),startTime=clean(input.startTime),endTime=clean(input.endTime),status=clean(input.status||existing.status||'planned');

...
export class InventoryStore extends DurableObject {
  ...
  submitQaCheck(body,actor){
    ...
    const failed=Object.values(results).includes('fail'),status=failed?(kind==='panel'?'recut':'rework'):'approved',notes=cleanText(body.notes,5000),photo=supportPhoto(body.photo),producer=kind==='panel'?source.cutBy:source.fabricatedBy,overrideReason=cleanText(body.overrideReason,500);
    ...
    this.ctx.storage.transactionSync(()=>{this.write('qa-checks',records);this.audit(actor.username,status==='approved'?'qa-approved':status==='recut'?'qa-recut-required':'qa-rework-required',{qaId:id,kind,project:source.project,orderNumber:source.orderNumber,reference:source.reference,producer,override:!!overrideReason,remakeOf:record.remakeOf||null});});

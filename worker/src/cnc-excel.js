...
export function connectCncWorkbook(files, headers, rows, url) {
  ...
  const rowCount=rows.length,tableEnd=Math.max(2,rowCount+1),tableRef=`A2:U${tableEnd}`,reports=buildCncReportRows(rows),excelDate=value=>{const match=value.match(/^(\d{2})\/(\d{2})\/(\d{4})$/),date=Date.UTC(Number(match[3]),Number(match[2])-1,Number(match[1]));return Math.floor((date-Date.UTC(1899,11,30))/86400000);};
  ...
  update('xl/workbook.xml','</sheets>',`<sheet name="Daily Report" sheetId="2" r:id="rId5"/><sheet name="Weekly Report" sheetId="3" r:id="rId6"/><sheet name="Monthly Report" sheetId="4" r:id="rId7"/></sheets><definedNames><definedName name="CNC_Tracker" localSheetId="0">'CNC Tracker'!$${tableRef.replace(':',':$').replace(/([A-Z])(\d+)/g,'$1$$$2')}</definedName></definedNames>`);
  ...
  update('[Content_Types].xml','</Types>','<Override PartName="/xl/connections.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.connections+xml"/><Override PartName="/xl/queryTables/queryTable1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.queryTable+xml"/><Override PartName="/xl/worksheets/sheet2.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/><Override PartName="/xl/worksheets/sheet3.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/><Override PartName="/xl/worksheets/sheet4.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/></Types>');
  ...
  update('xl/worksheets/sheet1.xml','</worksheet>','<ignoredErrors><ignoredError sqref="B2:C1048576 G2:G1048576 L2:U1048576" numberStoredAsText="1"/></ignoredErrors></worksheet>');
  ...
  const extras={
    ...
    'xl/connections.xml':`<connections xmlns="${ns}"><connection id="1" name="PanelStock CNC live" description="Read-only CNC schedule. Refreshes every minute while Excel is open. Enable this connection only if you trust PanelStock." type="4" refreshedVersion="8" refreshOnLoad="1" interval="1" saveData="1"><webPr xl2000="1" url="${xml(url)}" htmlTables="1" htmlFormat="all"/></connection></connections>`,
    ...
    'xl/queryTables/queryTable1.xml':`<queryTable xmlns="${ns}" name="CNC_Tracker" headers="0" backgroundRefresh="0" refreshOnLoad="1" connectionId="1" preserveFormatting="1" adjustColumnWidth="0" growShrinkType="insertDelete" applyNumberFormats="0" applyBorderFormats="0" applyFontFormats="1" applyPatternFormats="1" applyAlignmentFormats="0" applyWidthHeightFormats="0"/>`
  };

...
export function connectCncWorkbook(files, headers, rows, url) {
  ...
  const rowCount=rows.length,tableEnd=Math.max(2,rowCount+1),tableRef=`A2:R${tableEnd}`,reports=buildCncReportRows(rows),excelDate=value=>{const match=value.match(/^(\d{2})\/(\d{2})\/(\d{4})$/),date=Date.UTC(Number(match[3]),Number(match[2])-1,Number(match[1]));return Math.floor((date-Date.UTC(1899,11,30))/86400000);};
  ...
  update('xl/worksheets/sheet1.xml','</worksheet>','<ignoredErrors><ignoredError sqref="B2:C1048576 G2:G1048576 L2:R1048576" numberStoredAsText="1"/></ignoredErrors></worksheet>');

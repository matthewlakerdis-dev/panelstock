...
export function connectCncWorkbook(files, headers, rowCount, url) {
  ...
  const extras={
    ...
    'xl/tables/table1.xml':`<table xmlns="${ns}" id="1" name="PanelStock_CNC" displayName="PanelStock_CNC" ref="${tableRef}" totalsRowShown="0"><autoFilter ref="${tableRef}"/><tableColumns count="16">${headers.map((header,index)=>`<tableColumn id="${index+1}" name="${xml(header)}"/>`).join('')}</tableColumns><tableStyleInfo showFirstColumn="0" showLastColumn="0" showRowStripes="0" showColumnStripes="0"/></table>`,

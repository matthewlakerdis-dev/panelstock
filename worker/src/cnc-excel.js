...
export function connectCncWorkbook(files, headers, rows, url) {
  ...
  const extras={
    ...
    'xl/tables/table1.xml':`<table xmlns="${ns}" id="1" name="PanelStock_CNC" displayName="PanelStock_CNC" ref="${tableRef}" headerRowCount="0" totalsRowShown="0"><tableColumns count="${headers.length}">${headers.map((header,index)=>`<tableColumn id="${index+1}" name="${xml(header)}" queryTableFieldId="${index+1}"/>`).join('')}</tableColumns><tableStyleInfo showFirstColumn="0" showLastColumn="0" showRowStripes="0" showColumnStripes="0"/></table>`,
    'xl/queryTables/queryTable1.xml':`<queryTable xmlns="${ns}" name="CNC_Tracker" connectionId="1" refreshOnLoad="1" backgroundRefresh="1" headers="0" rowNumbers="0" preserveFormatting="1" adjustColumnWidth="1" growShrinkType="insertDelete" applyNumberFormats="0" applyAlignmentFormats="0" applyBorderFormats="0" applyFontFormats="0" applyPatternFormats="0" applyWidthHeightFormats="0"><queryTableRefresh nextId="${headers.length+1}" headersInLastRefresh="0" preserveSortFilterLayout="1"><queryTableFields count="${headers.length}">${headers.map((header,index)=>`<queryTableField id="${index+1}" tableColumnId="${index+1}" name="${xml(header)}"/>`).join('')}</queryTableFields></queryTableRefresh></queryTable>`
  };

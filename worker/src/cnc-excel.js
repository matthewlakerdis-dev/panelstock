...
export const CNC_COLUMNS=['Project','Order number','Sheet number','Length (mm)','Width (mm)','Sheet area (m²)','Panel IDs','Panel area (m²)','Waste','Status','Off-cut saved','Off-cut details','Uploaded by','Date uploaded','Time uploaded','Completed by','Date completed','Time completed'];
...
export function buildCncExcelRows(panels,splitDateTime) {
  ...
  return [...grouped.values()].map((group,index)=>{
    ...
    return {'Project':group.project,'Order number':group.orderNumber,'Sheet number':group.sheetNumber,'Length (mm)':group.length||'','Width (mm)':group.width||'','Sheet area (m²)':{formula:`D${row}*E${row}/1000000`,value:group.length*group.width/1000000},'Panel IDs':panelIds,'Panel area (m²)':panelArea||'','Waste':{formula:`IF(F${row}>0,MAX(0,(F${row}-H${row})/F${row}),"")`,value:group.length&&group.width?Math.max(0,(group.length*group.width/1000000-panelArea)/(group.length*group.width/1000000)):''},'Status':completed?'Completed':'Pending','Off-cut saved':completed?(savedOffcut?'✓':'✕'):'','Off-cut details':offcutDetails,'Uploaded by':uploadedPanel.uploadedBy||'','Date uploaded':uploaded.date,'Time uploaded':uploaded.time,'Completed by':completedPanel.completedBy||'','Date completed':finished.date,'Time completed':finished.time};
  });
}
...
export function connectCncWorkbook(files, headers, rows, url) {
  ...
  update('xl/worksheets/sheet1.xml','</worksheet>','<ignoredErrors><ignoredError sqref="B2:C1048576 G2:G1048576 M2:R1048576" numberStoredAsText="1"/></ignoredErrors><tableParts count="1"><tablePart r:id="rId1"/></tableParts></worksheet>');

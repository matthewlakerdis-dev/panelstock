...
test('hosted LibreOffice converter is private, bounded and keeps a PDF fallback',async()=>{
  ...
  assert.match(analyser,/DEFAULT_CUT_EDGE_ALLOWANCE_MM = 10/);
  assert.match(analyser,/minimum_offcut_size/);
  assert.match(server,/X-CNC-Cut-Edge-Allowance-MM/);
  assert.match(server,/X-CNC-Minimum-Offcut-Size-MM/);
  assert.match(worker,/X-CNC-Cut-Edge-Allowance-MM/);
  assert.match(worker,/X-CNC-Minimum-Offcut-Size-MM/);

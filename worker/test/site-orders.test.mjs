...
test('site order app parses and keeps its own offline queue',()=>{
 ...
 assert.match(app,/class="card account-summary"/);
 assert.match(app,/current\.displayName\|\|session\.username/);
 assert.match(app,/class="account-avatar"/);
...
test('site orders use the simplified status filters and PDF and Excel actions',()=>{
 ...
 assert.doesNotMatch(app,/if\(event\.target===overlay\)overlay\.remove/);
 assert.match(app,/class="date-trigger" data-date-picker[^`]+<svg viewBox="0 0 24 24"/);

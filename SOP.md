...
Open https://app.panelstockhq.com on your phone or tablet. Navigation is along the bottom; the stock tab is labelled SOH. Use your own account so receipts, dispatches and CNC completions are attributed to you. On a shared device, sign out when finished and turn off Remember my username if appropriate.
...
1. Open SOH. Switch between Full panels and Off-cuts. Search for the material or SKU and check colour, thickness and size before taking stock.
...
1. Open SOH, choose Off-cuts and select the add-offcut control.
...
1. Open CNC. For one panel, use Schedule; enter the order, optional job reference, sheet and panel ID, then add it to the schedule.

2. For several panels, use Schedule multiple panels. Enter Order number and Job reference once; both are required for bulk entry.
